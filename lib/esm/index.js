...
function startMacros({ macros, dimensions, updatePixels, }) {
    const stops = macros.map(({ macroName, macroConfig }, macroIndex) => {
        if (macroName === MacroName.Box) {
            return startBox(Object.assign({ color: "#fff", startingColumn: 0, startingRow: 0, width: dimensions.width, height: dimensions.height, brightness: 10 }, macroConfig), macroIndex, updatePixels);
        }
