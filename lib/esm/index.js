...
function startMacros({ macros, dimensions, updatePixel, }) {
    const stops = macros.map(({ macroName, macroConfig }, macroIndex) => {
        if (macroName === MacroName.SolidColor) {
            return startColor(Object.assign({ color: "#fff", startingColumn: 0, startingRow: 0, width: dimensions.width, height: dimensions.height, brightness: 10 }, macroConfig), macroIndex, updatePixel);
        }
        if (macroName === MacroName.Text) {
            return startText(Object.assign({ color: "#fff", text: "hello WORLD!", font: "system-16", alignment: "left", spaceBetweenLetters: 1, spaceBetweenLines: 1, startingColumn: 0, startingRow: 0, width: dimensions.width, brightness: 10 }, macroConfig), macroIndex, updatePixel);
        }
        if (macroName === MacroName.Twinkle) {
            return startTwinkle(Object.assign({ color: "#FFF", speed: 100, width: dimensions.width, height: dimensions.height, brightness: 10 }, macroConfig), macroIndex, updatePixel);
        }
        if (macroName === MacroName.MeteorShower) {
            return startMeteorShower(Object.assign({ color: "#FFF", meteorCount: 40, maxTailLength: 20, minTailLength: 5, maxDepth: 5, minSpeed: 100, maxSpeed: 10, width: dimensions.width, height: dimensions.height, brightness: 10 }, macroConfig), macroIndex, updatePixel);
        }
        if (macroName === MacroName.Marquee) {
            return startMarquee(Object.assign({ color: "#fff", text: "Replace with marquee text!", font: "system-16", speed: 50, width: dimensions.width, height: dimensions.height, brightness: 10 }, macroConfig), macroIndex, updatePixel);
        }
        ...
        if (macroName === MacroName.Time) {
            return startTime(Object.assign({ color: "#fff", font: "system-6", alignment: "left", spaceBetweenLetters: 1, spaceBetweenLines: 1, startingColumn: 0, startingRow: 0, width: dimensions.width, brightness: 10 }, macroConfig), macroIndex, updatePixel);
        }
        return startText(Object.assign({ color: "#fff", text: "unsupported", font: "system-6", alignment: "left", spaceBetweenLetters: 1, spaceBetweenLines: 1, startingColumn: 0, startingRow: 0, width: dimensions.width, brightness: 10 }, macroConfig), macroIndex, updatePixel);
    });
    return async () => {
...
export function createDisplayEngine({ dimensions = { width: 23, height: 128 }, onPixelChange, }) {
    let stopMacros = () => { };
    return {
        render: (macros) => {
            stopMacros();
