...
export const startMeteors = async ({ macroConfig, dimensions, index, ctx, updatePixels, }) => {
    const config = Object.assign({ color: "#FFFFFF", meteorCount: 40, maxTailLength: 20, minTailLength: 5, maxDepth: 10, minSpeed: 100, maxSpeed: 10, width: dimensions.width, height: dimensions.height, brightness: 10 }, macroConfig);
    ...
    const generateMeteor = () => {
        const tailLength = Math.floor(Math.random() * (config.maxTailLength - config.minTailLength)) + config.minTailLength;
        ...
        return {
            ...
            speed: Math.floor(Math.random() * (config.minSpeed - config.maxSpeed)) +
                config.maxSpeed,
            depth,
            moveCount: 0,
            complete: false,
        ...
        meteors.forEach((meteor, i) => {
            ...
            if (meteors[i].moveCount > meteor.speed) {
                ...
                meteor.path.forEach((dot, i) => {
                    const meteorDotAlphaPercentage = (meteor.tailLength - i) / meteor.tailLength;
                    ...
                    d[3] =
                        meteorDotAlphaPercentage *
                            ...
                            (rgba === null || rgba === void 0 ? void 0 : rgba.a);
